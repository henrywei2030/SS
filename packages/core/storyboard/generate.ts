...
export interface GenerateStoryboardInput {
  ...
  /**
   * H1(docs/07):分镜侧知识注入块 — router 层调 buildSceneKnowledgeBlock(按场原文
   * keyword 检索 + 项目世界观条目)后传入;缺省/null = 不注入(向后兼容)。
   * 本函数不查库,保持纯编排(注入内容由调用方负责)。
   */
...
export interface GeneratedShot {
  ...
  // 六八:四维电影级 — 音效设计(环境底+动效+强调,自由文本不受预设限制)
  sound?: string;
...
// 六八:导出供 DB 模板更新脚本 / seed-core 对账用(原仅 loadPromptTemplate fallback)
export const SYSTEM_PROMPT = `你是电影摄影指导(DoP)出身的短剧分镜师。任务：把单场剧本拆解为视频生成可用的分镜列表,为每一镜做出电影级的「景别角度 / 运镜 / 光影 / 音效」四维设计 — 每个选择都要有叙事动机,并保证上下镜衔接连贯。
...
■ 运镜(movement):
- 无动机不动机位:推=聚焦/逼近真相;拉=揭示环境/孤立感;摇=视线引导/空间扫描;移/跟=伴随人物/追逐紧张;升降=命运感/段落转场;甩=暴力转场/时间跳跃
...
【提示词写作 — 公式：景别角度 + 主体(细节) + 动作(速率) + 场景(层次) + 光影 + 氛围 + 运镜 + 音效 + 台词】
- 起手：景别 + 角度 + 主体；主体带关键细节(外貌/服装/表情)
- 动作写清速率(缓缓 / 猛地 / 急促)；场景写层次(前景 ··· 背景)
- 光影写方向与质感;运镜写进末段;音效用一短句融入画面(如"雨点击打铁皮棚顶,闷雷渐近")
- 台词放在末尾，格式 "角色名：台词"
- OS 旁白格式 "角色名（OS）：旁白文字"
...
示例正确输出(直接复制此结构填值):{"shots":[{"index":1,"framing":"特写","angle":"平视 0°","movement":"固定","lighting":"低调","sound":"雨声渐强,心跳声起","content":"...","durationS":3,"priority":"B","prompt":"..."}]}`;
...
function buildUserPrompt(input: GenerateStoryboardInput): string {
  ...
  const meta = [
    ...
    styleBlock ? `\n【全剧风格规约】\n${styleBlock}` : null,
    presetBlock ? `\n【可选预设】\n${presetBlock}` : null,
    // H1:分镜侧知识注入(块内自带【创作知识】标题,router 检索后传入)
    input.knowledgeBlock ? `\n${input.knowledgeBlock.trim()}` : null,
...
function extractShots(
  ...
  const items = root.shots
    .map((raw, i): GeneratedShot | null => {
      ...
      // 六八:音效自由文本,空串视为 undefined;裁 60 字防 LLM 写小作文
      const sound =
