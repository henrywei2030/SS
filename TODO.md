...
- [x] **Binding 强制显式选(收工后补丁)**— 用户反馈"测试调试可以,实际用必须后台设置"/ seed.ts 7 binding 默认值改 ''(留 docx.parser) / 5 业务 router fallback 改空时抛 PRECONDITION_FAILED + 引导 /admin/bindings / DB SQL UPDATE 修 8 行 / ADR-28 §F 落地 explicit-choice-only 原则 / smoke 19/19 保持过 — 2026-05-24
