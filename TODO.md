...
> 最后更新:2026-06-04(**五二收工 · 分镜结果对话框配色token化+失败就地重试+进度条改绿 + 生成永不自动组(代码定死) + 解释分镜组合并原理**)
...
- [x] **分镜生成结果对话框配色+失败重试+进度条改绿 + 生成永不自动组(代码定死) + 解释合并原理**(五二收工 · mac-mini · 2026-06-04)— 用户截图反馈。**①结果对话框配色+重试**(`top-bar.tsx` 五十收工「全部集数生成」):chip/进度条/失败列表硬编码emerald/pink/red→design token(`--color-success/destructive/primary`)协调;进度条改绿(`--color-success`);抽`runPool`复用3并发池+`retryFailed`+「重新生成失败的N集」按钮(`failedEps>0`显示,只重跑失败集)。**用户实测**:限流失败的6/12/18集重试后全成功。**②生成永不自动组**(代码定死,用户强调规则):根因非bug=mac-mini本机DB`storyboard.autoMergeOnGenerate=true`(代码/seed默认false,各机漂移mac-studio false/mac-mini true);移除`storyboard.ts` generateForEpisode自动`mergeShots`合并段(~60行)+autoMerge binding/import+seed setting;生成永远单镜(`createdGroupIds`恒空),人工组合(手动`mergeShots` endpoint+`splitGroup`保留),发布仍1:1 group化供AIGC;`merge.ts`算法+test保留(core能力future可接)。**③解释合并原理**:`mergeShots`=按`maxDurationS`向下贪心合并相邻镜(累加不超阈值就并,超则开新组)。typecheck 20/20+test全绿。**留**:`merge.ts`成生产孤立代码(只test用)是否删待定 / 旧组不动只影响新生成 / mac-mini DB autoMerge=true已失效
- [x] **前两轮漏洞两遍审查+4项加固 + 项目编辑功能(列表行)+ 关联剧本导入0集根因修 + Chrome 真打**(五一收工 · mac-mini · 2026-06-04)— 开工指令"回顾前两轮(五十/四九)全面检查两遍漏洞"。**①两遍审查**:第一遍亲自精读8文件diff + 第二遍2独立agent交叉验证(访问控制/经济/并发 + 注入/泄露/密钥/XSS/DoS),三方收敛**无P0/P1**(IDOR/经济/密钥/ReDoS/XSS逐项排除)。用户选全修**4加固**:脱敏(`sanitizeErrorMsg`补JWT/Bearer/sk-key/裸IP,原漏带连字符key;实测9case+ReDoS 10ms)/ 小卫生(`episodeNumbers .max(200)`+catch只接CONFLICT)/ 并发锁(`generateEpisode`+`generateAllEpisodes` draft.episodes套advisory-lock`insp_draft:`,LLM锁外跑锁内重读防丢集)/ text budget guard(`checkTextBudget`+seed`text.generate.dailyBudgetCny`默认0)。**②Chrome真打**:`pnpm dev`起系统,灵感创作第1集screenplay正确+全部展开pending=0+12集完整。**③项目编辑功能**(新实现):`create-project-dialog`改创建/编辑双模式+`projects-list`行hover编辑按钮(阻止行导航);真打预填+保存生效;**真打修bug** `description||undefined`致编辑清空无效→编辑传实际值。**④关联剧本导入0集根因修**(用户报):清空(软删)后重关联,`upsert`命中软删Episode漏复活`deletedAt:null`→列表查不到→0集;修`script.ts` 3处(link+单集content/docx upload)加`deletedAt:null,status:NOT_STARTED`对齐`uploadMultiEpisode`;真打0→12集。typecheck 20/20+test全绿。**留**:漏洞审查盲区(0集复活bug两遍审查漏掉,只盯安全维度+轻信"复用"未逐行对比)/ budget默认0需配非零生效 / 端到端新链路真打
