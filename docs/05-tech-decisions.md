...
- ✅ **P3(部分)** `3bf7816`:**先补测试再去重**(用户拍板的安全路径)—— 新增 `utils/generation-attempt.ts` 的 `runTextGenerationAttempt`(RUNNING→SUCCESS/FAILED 状态机)+ 4 单测锁行为,收敛 **inspiration 3 处**手写样板(无 CostLedger,最安全先行;行为保持,仅解析失败 errorMsg 归一)。
  - **P3 剩余**:asset-generate(generateProfileField/AssetText/Image)、asset-breakdown、storyboard-generate 等带 **CostLedger + $transaction** 的样板 —— billing 关键,需先扩 helper(或抽 `writeLedgerEntry`)+ 各自测试,再逐处增量。EventBus 删除待核 ADR-26/27 是否 Phase-2 占位。
