...
export class BaseProvider {
    /** 记录 Cost Ledger（同步落库） */
    async recordLedger(opts) {
        if (opts.ctx.skipLedger)
            return;
    ...
    async checkBudget(projectId, estimatedCost) {
        if (!projectId)
            return;
        ...
        if (usedAmount + estimatedCost > limit) {
            throw new BudgetExceededError(`project ${projectId}`, limit, usedAmount + estimatedCost);
        }
    }
    /** 包装远程调用 — 统一错误转换 */
    wrapCallError(e) {
        if (e instanceof ProviderError || e instanceof BudgetExceededError)
            throw e;
