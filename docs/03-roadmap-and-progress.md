...
| **W3 分镜工坊** | 🚧 后端 + 主 UI 完成 | 85%（W3.0-W3.5 ✅ / W3.6-W3.7 🚧） |
| **W4-W8** | 📋 排期 | 0% |
| **Phase 2** | 📋 待启动 | 数据模型与接口已预留 |
| **Phase 3** | 🔮 远期 | Schema 字段已预留 |

**整体可升级性评级：A**（详见 `PROGRESS.md` 中 W1-W2 审计记录）
...
| **W3** | Storyboard Studio | 🚧 85% | 数据底座 + 11 procedures + 多格式上传 + 三栏 UI + 合并拆分;**W3.6 行内编辑 + W3.7 polish 剩余** |
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| 多 Agent token 爆炸 | 高 | LangGraph 显式状态机 + Token Budget + Critic 早停 |
