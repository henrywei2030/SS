...
- **每个会话都必须开启远程控制(2026-06-10 六六收工新增)**:会话一开始(**不限于开工**,任何对话)第一时间提醒我运行 `/remote-control`(UI 命令,Claude 无法代跑),保持全程可从手机/别处远程查看·控制(详见行为准则 9)
...
#### Step 0.5:开启远程监控(`/remote-control`)
开工即**提醒我运行 `/remote-control`**(⚠️ 这是 **UI 命令,Claude 无法代跑**,须我在界面亲自输入),
开启「**可从手机 / 别处远程查看 · 控制本会话**」。尤其长时自主任务(打包 / CI / 批量真打)时,
我离开电脑也能远程盯进度、随时介入或中断。
...
#### Step 10:远程监控收尾(`/remote-control`)
收工时**提醒我再运行一次 `/remote-control`**(UI 命令,Claude 无法代跑),确保本次收工状态
远程可见(跨设备 / 手机查看),并让后续若有自主收尾任务仍可远程介入。
...
9. **远程控制全程开启(2026-06-10 六六收工新增)**:**所有对话**都需处于远程可控状态 — 每次会话开始(开工、临时问答、长时自主任务皆同)第一时间提醒我运行 `/remote-control`;收工 Step 10 再确认一次;会话中途重开同样先提醒。这是 UI 命令,Claude 永远无法代跑,只能提醒
