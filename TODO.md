...
> 最后更新:2026-06-15(**mac-studio · 七二第十波**:生成分镜 0 场根因【replaceExisting 顺序 bug】+ AIGC 视频 blocker【audio_url 可达性】+ 风格体系重命名【2D/3D/AI伪真人】扩 9 风格【全去 IP】+ 项目↔风格联动接通 + 分镜 prompt 规范化【@作用域/名词一致性】+ 知识库批量导入 + **参考声线 mp3/base64 内联整套打通**【真打 Seedance create 成功】 · 详见 [PROGRESS](PROGRESS.md))
> 上一版:2026-06-14(**mac-studio**:依赖大升级接续 + 导演模块 **v0.2.0** 链路重构【拆解换源=分镜脚本快照 / 向导式流水线 stepper / 美术侧拆解统一下线】+ 3 遍审核 17 项优化 + Mac dmg v0.2.0 打包(独立运行验证 PASS)+ 手机端方案 B Phase0/1【暂缓封存】)
...
- [x] **🎬🎨 七二第十波(2026-06-15 mac-studio · 用户报/指令)· 生成分镜0场 + AIGC五诉求 + 参考声线打通**:① **生成分镜出 0 场根因** = replaceExisting 级联软删放在取场后(删场不重建 → shot 孤儿)→ 删除挪到取场前;间歇「LLM 未返回 JSON」加非截断重试。真打 ep1「4 场 26 镜」。② **版本字段一致性**(health fallback 0.1→0.2)+ **logo 放大**(lg size-20→24)。③ **AIGC #1 视频 blocker** audio_url 加 isRelayFetchableUrl 过滤(A+duelB)+ 错误 slice 200→600;**#2** adapter 防御去重+图≤9/音≤3;**#3** @人名混进剧本内容(extractShots 剥 content @ + 清存量 81)+ SYSTEM_PROMPT/seed 规范化(@作用域/名词一致性/seedance 精简);**#4** 风格改名 **2D/3D/AI伪真人** + **并行 agent 撰 9 新风格全去 IP**(日式/美式/游戏CG/水墨/赛博朋克/手绘水彩/黏土/像素/油画,DB 12 风格零 IP)+ **接通项目↔风格联动**(type 自动绑+选择器+回填,治 styleId 全 NULL 风格 prompt 不生效)+ zhStyle/KIND_LABELS/i18n 同步;**#5** 知识库 importBatch + 批量导入 UI + seed 10 条语料(83→93)。④ **参考声线真正打通**(比原 relay-only 更好):**真打证 moyu audio_url 支持 base64 内联** → TTS/规范化产物 m4a→mp3(ffmpeg 按扩展 libmp3lame)+ compile 本地声线内联 base64 + 格式(mp3/wav)/时长(≥1.8s)守门 + isRelayFetchableUrl 放行 data:/asset:// + syncToRelay 默认 false + 存量 8 声线转码。**真打 Seedance create 成功**(base64 mp3+图)。typecheck core/api/adapters/web 全 0,53 测试过。**留**:worker 需重启吃新代码(已重启本机);各机 `db:sync:prompts`;用户真打确认出片。详见 [PROGRESS](PROGRESS.md) + 记忆 [[seedance-base64-audio]]。
- [x] **🎬 导演模块 v0.2.0 链路重构(2026-06-14 mac-studio · 用户指令)**:多智能体深挖现状 → 3 系统设计方案(用户选**方案丙 向导式流水线**)→ 4 阶段全落地 + 真打验证 + 3 遍审核优化。**① 阶段A 地基**:新表 `StoryboardExport`(分镜脚本快照,migration `add_storyboard_export`)+ `storyboard.pipelineStatus`(导演流水线单一真相)+ `loadProjectShootingScript`(带 Scene.content 的分镜脚本构建,缓解镜头碎片化)。**② 阶段B 导出**:`storyboard.exportScript`/`listExports` + 分镜工坊「集数多选导出」Dialog + 全集完成门禁。**③ 阶段C 拆解换源**:`breakdownProject` 接 `exportId`(拆解输入=分镜脚本快照,**非剧本原文** — 修正用户报的「拆解来源」逻辑错误)+ 真打质量优秀(陆鸣/小A 深度档案;**结论:镜头 prompt 自带上下文,无需新 prompt**)+ 拆解 pane 文案/门禁 + **统一美术侧口径**(下线 `art/breakdown-dialog`,3 触点改跳导演)+ 删错位「拆解来源」面板。**④ 阶段D 流水线收口**:tab 重排 灵感→剧本→**分镜→拆解** + pipelineStatus 驱动阶段状态(✓/🔒)+ 箭头 stepper + 两入口默认 tab 对齐。**E2E 真打全链路打通**:导出快照→拆解→入库→syncToArt→美术可见。**3 遍审核**(24 agent + 对抗复核)17 项优化全应用(真 bug:pipelineStatus 漏 deletedAt 过滤;删 breakdown-review-dialog 不可达 else 死分支 ~85 行 + 死 prop/字段;陈旧注释/docstring 纠偏;3 文档归档横幅)。版本号 → **v0.2.0**。typecheck 16/16。**留**:可选打磨(STAGES 单一配置 / `?tab=shots&sub=breakdown` 子 tab 嵌套 / 每阶段 CTA);单集 `asset.breakdown`+`breakdownAssets` 已标 DEPRECATED 待评估删。
- [ ] **⏸ 手机端 UI 改造 · 方案 B 沉浸暗色工作室(2026-06-14 mac-studio · 暂缓中)**:三方案设计(A 极简留白 / **B 沉浸暗色工作室【已选】** / C 卡片化 Bento,含多智能体研究 Mobbin/Muzli/NN/g/Material/Linear)→ 用户选 B → 已落地 **Phase 0 基座**(globals.css 移动断点/`dvh`/safe-area/第四级表面/`.glow-*`/`glow-pulse`/`.glass-bar`/reduce-motion + viewport `viewport-fit=cover`、themeColor `#1F1F1F`)+ **Phase 1 登录页**(片场暗房辉光/44pt 输入/蓝色辉光 CTA/footer 溢出修复,**浏览器已验证**)+ **Phase 1 底部 Tab Bar/中置辉光 FAB/移动 header**(`components/mobile-nav.tsx`,代码完成、视觉复验被打断)。**用户指令暂缓此方向**,转处理导演模块 v0.2.0。**完整封存** → [docs/09-mobile-ui-redesign-plan-B.md](docs/09-mobile-ui-redesign-plan-B.md)(三方案/研究出处/已动 7 文件/待办 Phase 1 首页+Phase 2 三工作区+收尾;admin 密码已恢复 seed 默认)。已设每日提醒。
- [x] **🔌 灵感创作 API 失败定位 + OpenAI-compat 文本重试 + 桌面端 Chrome 调试(2026-06-14 win-laptop 第六场)· 用户报**:灵感创作「生成大纲」失败,**全程取证确证非配置问题** —— 查桌面内嵌库证 moyu 中转站/key/binding 全对;真 key 复现 6 次 **5 成 1 败**,失败 = `socket disconnected before secure TLS`(建连期 ECONNRESET);DNS 解析 `www.moyu.info→198.18.2.44`(代理 fake-ip)。**根因 = 经代理到 moyu 的间歇建连重置 + app 文本生成无重试**(单次撞上即判死)。**修**:[openai-compat.ts](packages/adapters/provider/openai-compat.ts) 加瞬时错误有限重试(`attemptOnce`+`requestWithRetry`,退避 [600,1800]ms;仅重试网络抛错码 + HTTP 429/5xx,**4xx 如 401 立即抛不掩盖配置错**;失败 attempt 不记 ledger)→ 根治所有走中转站文本功能(灵感/剧本分析/分镜/QC/拆解)。单测 20/20 + typecheck 绿。桌面端 Chrome 打开,顺带揪出 **Next standalone 仅绑 IPv6 `::1`**(IPv4 探测 ECONNREFUSED 真因)。**留**:① 旧包无重试→打新包才生效(眼下缓解:代理 `moyu.info`设直连 / 重试按钮)② standalone 绑双栈 backlog ③ Mac 包须在 mac 机打(Win 不能交叉编译)
- [x] **🖥️ 卡 splash 终极根因 + 转 NSIS 单用户 + splash 进度条 + 7 项 app 修复(2026-06-14 win-laptop 第五场)· 用户指令**:真机实测层层逼出**最深根因 strip_unc** —— Tauri `resource_dir()` 返回 `\\?\` verbatim 路径,node 解析不了 → `EISDIR lstat 'C:'` → sidecar 启动即崩 → 卡 splash(挡在 chmod/stdio 前,手动测试用干净路径一直绕开)。修:`main.rs` `strip_unc()` 去前缀 + sidecar stdio→`sidecar.out.log`(治 GUI 无控制台无效 stdio 崩 + 兜底)。**转 NSIS 单用户安装**(`installMode:currentUser` → 装可写 `%LOCALAPPDATA%`、无需管理员,从根上消除 Program Files 只读类错误,含 chmod EPERM;chmod patch 留兜底)。**splash 实时进度条**(boot-progress.json→main.rs 轮询→splash)。**7 项 app**:系统性 **confirm sweep**(WebView2 吞 native confirm/alert → `useConfirm` hook 改 7 文件,修 #1b provider 删除/#2 视频删除桌面失效)+ #1a seed 清空默认直连(冒烟「0 Provider」)+ #4 占位文案 + #6 顶栏三项移入用户下拉 + #7 Projects→项目名 + #3 性能(verifyToken 30s 缓存/placeholderData/loading 骨架/登出去 refresh)。#5 登录持久分析为坏包遗留,working 包待验。决定性验证:真机静默装+GUI 跑通激活页 200。typecheck 16/16。决策 [ADR-38](docs/05-tech-decisions.md)。**留**:真机走 7 项验收(尤其 #5)
- [x] **🖥️ Win 安装包重打 + 卡 splash 双根因修复 + 全盘验收(2026-06-14 win-laptop 第四场)· 用户指令**:取证调查(4 路 workflow)+ `--once` 冒烟实捕,"卡初始界面"= **两个独立 bug**:① **VC++ CRT 缺失**(全新机 postgres.exe 缺 vcruntime140.dll → 内嵌 PG 起不来;构建机 BuildTools 掩盖)→ **app-local CRT** 拷进 PG bin(exe 目录优先 System32,打包器无关) ② **seed shell 转义**(`run()` shell:true 跑含空格 `process.execPath` 被 cmd 拆断,**所有 Win 机中招、既有 bug**)→ 绝对路径不走 shell。另修 **MSI MAX_PATH**(`next@16.2.9` `.pnpm` 超长路径 LGHT0103 → 删 hoist 后冗余 `.pnpm`,瘦身 45MB)。配置:`targets:["msi"]` + WebView2 `offlineInstaller`(离线内嵌)。诊断:desktop.log 第一行落盘 + last-error 回显 splash。**验收三项全 PASS**:依赖齐全 / `msiexec /a` exit 0 / 端到端进激活页 200(非 splash)。MSI 430MB。决策 [ADR-37](docs/05-tech-decisions.md) · 工具 `scripts/verify-desktop-flow.mjs`。**留**:干净无 VC++ 机实装确认 CRT 修复真生效
- [x] **🔒 全盘深度审计 + 21 漏洞修复(2026-06-14 win-laptop 第三场)· 用户指令**:① **依赖** `pnpm audit` 4 CVE→0(esbuild0.28.1 + postcss/@hono override)② **模块逻辑** 分层干净无循环、零结构 bug ③ **传导链路** 6 条端到端全通 ④ **代码漏洞×3遍**(58 agent/3.78M tokens)37 raw→**21 确认**(毙 16 误报)→ **全修复**:批①认证/密钥(logout CSRF / 弱KDF prod 拒 / 脱敏统一 / 解密日志)批②SSRF(refVideo/AudioURL refine / **validateApiUrl IPv6 方括号绕过真 bug** / isRelayFetchableUrl 硬化)批③DoS(DOCX zip bomb / base64 上限 / 集数上限治 N×DB)批④临时文件 try-catch。1 误报(字符正则)读码不动。5 commit · typecheck 16/16 · test 12/12。**留 backlog**:非-bug 设计债(ShotAssetRef deprecated / dist/cost 死产物 / recordLedger 写失败只日志 / DNS rebinding Phase2)
...
- [ ] **🖥️ 桌面程序化遗留**(ADR-35;六八:**dmg 缺四原生依赖的六七债已修**(依赖闭包补包+darwin-arm64 裁剪,238M 残→300M 全)+ 构建隔离 .next-desktop(打包不再打断 dev server))— 剩:① ✅ **第四场:win-laptop 本地出含修复 MSI(VC++ app-local/seed/.pnpm/WebView2 离线,ADR-37),全盘验收三项 PASS;剩干净无 VC++ 机实装确认** ② 桌面包 Mock 视频端到端 + TTS 权重首跑真打 ③ 退出钩子加固 ④ .dmg Finder 美化/自动更新(低优)⑤ Developer ID + 公证(分发才需要)
